# Tranche J — Page de suivi publique `/suivi/[id]`

## Contexte

La page `merchant/commande/[id]` permet au marchand de suivre une livraison en temps réel. Elle contient un bouton "Partager le suivi par WhatsApp" qui envoie `window.location.href` — c'est-à-dire une URL protégée par auth. Le client qui reçoit ce lien se retrouve redirigé vers `/login`.

Cette tranche crée une page publique `/suivi/[id]` accessible sans authentification, et met à jour le lien WhatsApp pour pointer vers cette nouvelle route.

## Objectif

- Une URL `/suivi/[id]` accessible sans login — le client peut voir sa livraison
- Affiche : carte Leaflet + statut réel + GlovoTimeline + ETA
- N'affiche pas : montant, paiement, numéro livreur, chat — rien de sensible
- Le statut est lu depuis `useOrdersStore` — se met à jour si l'admin change le statut

## Architecture

Deux fichiers. La route `/suivi` n'est pas dans le `matcher` du middleware (`/admin/:path*`, `/merchant/:path*`, `/driver/:path*`) — elle est publique par défaut sans aucun changement au middleware.

```
app/suivi/[id]/page.tsx        ← Créer — page publique Client Component
app/(merchant)/merchant/commande/[id]/page.tsx  ← Modifier — URL WhatsApp
```
...
| `app/(merchant)/merchant/commande/[id]/page.tsx` | Modifier — lien WhatsApp → `/suivi/${params.id}` |
...
## `app/(merchant)/merchant/commande/[id]/page.tsx`

Une seule ligne à modifier — le `waText` :
...
// Après
const waText = encodeURIComponent(`Suis ta livraison YONNE en temps réel 🛵 ${typeof window !== "undefined" ? `${window.location.origin}/suivi/${params.id}` : ""}`);
```
...
- Pas de rafraîchissement automatique (polling) — le client recharge manuellement
- Pas d'affichage du numéro de livreur
- Pas de chat
- Pas de montant ni mode de paiement
- Le middleware n'est pas modifié
